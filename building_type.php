...
{
var building_typename=document.getElementById("building_type").value;
if(building_typename=="")
{
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `building_type` where name like '%".$trimmed."%'";
}
else
{ 
	$qry="SELECT *  FROM `building_type`"; 
}
...
$query = "SELECT * FROM building_type where name = '$building_type'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("building_type.php?success=exists");
		}
		else
		{
			if ($building_type != "")
			{
			if(!mysql_query('INSERT INTO building_type (name,created_by)VALUES ("'.$building_type.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("building_type.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("building_type.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($building_type != "")
			{
			if(!mysql_query('UPDATE building_type SET name="'.$building_type.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("building_type.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("building_type.php?success=error");
...
{
$id=$_GET['id'];
$query = "SELECT * FROM building_type where id=$id"; 
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Building Type</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100">Building Type*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='building_type' id='building_type'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='building_type' id='building_type'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Building Type'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Building Type<img src="images/sort.png" width="13" height="13" /></th>
			...
			<tr>
				...
				<td align="right">
				<a href="building_type.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
