...
if(isset($_GET['delete_id']) && intval($_GET['delete_id'])) {
	...
	if(!mysql_query("delete FROM vendor where id=$id")) {
		die('Error: ' . mysql_error());
	}
	$fgmembersite->RedirectToURL("view_vendor.php?success=delete");
	// 
	//echo "hi";
	}
}
...
<span style="float:left;"><input type="button" name="kdproduct" value="Add vendor" class="buttonsbig1" onclick="window.location='vendor.php'"></span><span style="float:right;"><input type="button" name="kdproduct" value="Close" class="buttons" onclick="window.location='ams_temp.php?id=3'"></span>
