...
function myFunction()
{
document.getElementById("allocation_type").value="";
document.getElementById("allocation_type").focus();
return false;
}
</script>
<script>
...
{
var allocation_typename=document.getElementById("allocation_type").value;
if(allocation_typename=="")
{
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `allocation_type` where name like '%".$trimmed."%'";
}
else
{ 
	$qry="SELECT *  FROM `allocation_type`"; 
}
...
$query = "SELECT * FROM allocation_type where name = '$allocation_type'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("allocation_type.php?success=exists");
		}
		else
		{
			if ($allocation_type != "")
			{
			if(!mysql_query('INSERT INTO allocation_type (name,created_by)VALUES ("'.$allocation_type.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("allocation_type.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("allocation_type.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($allocation_type != "")
			{
			if(!mysql_query('UPDATE allocation_type SET name="'.$allocation_type.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("allocation_type.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("allocation_type.php?success=error");
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Allocation Type</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100">Allocation Type*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='allocation_type' id='allocation_type'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='allocation_type' id='allocation_type'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Allocation Type'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<td align="right">
				<a href="allocation_type.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
