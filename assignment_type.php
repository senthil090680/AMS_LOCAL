...
{
var assignment_typename=document.getElementById("assignment_type").value;
if(assignment_typename=="")
{
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `assignment_type` where name like '%".$trimmed."%'";
}
else
{ 
	$qry="SELECT *  FROM `assignment_type`"; 
}
...
$query = "SELECT * FROM assignment_type where name = '$assignment_type'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("assignment_type.php?success=exists");
		}
		else
		{
			if ($assignment_type != "")
			{
			if(!mysql_query('INSERT INTO assignment_type (name,created_by)VALUES ("'.$assignment_type.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("assignment_type.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("assignment_type.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($assignment_type != "")
			{
			if(!mysql_query('UPDATE assignment_type SET name="'.$assignment_type.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("assignment_type.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("assignment_type.php?success=error");
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Assignment Type</div>
...
<table>
  <tr height="60px">
	 ...
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='assignment_type' id='assignment_type'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='assignment_type' id='assignment_type'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Assignment Type'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Assignment Type<img src="images/sort.png" width="13" height="13" /></th>
