...
{
var uomname=document.getElementById("uom").value;
if(uomname=="")
{
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `uom_ams` where name like '%".$trimmed."%'";
}
else
{ 
	$qry="SELECT *  FROM `uom_ams`"; 
}
...
$uom=$_POST['uom'];
$query = "SELECT * FROM uom_ams where name = '$uom'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("uom_ams.php?success=exists");
		}
		else
		{
			if ($uom != "")
			{
			if(!mysql_query('INSERT INTO uom_ams (name,created_by)VALUES ("'.$uom.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("uom_ams.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("uom_ams.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($uom != "")
			{
			if(!mysql_query('UPDATE uom_ams SET name="'.$uom.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("uom_ams.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("uom_ams.php?success=error");
...
{
$id=$_GET['id'];
$query = "SELECT * FROM uom_ams where id=$id"; 
...
<table>
  <tr height="60px">
     <td class="pclr" width="100">Uom*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='uom' id='uom'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='uom' id='uom'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
?> </h3><a href="<?php echo $_SERVER['PHP_SELF']; ?>"><button id="closebutton_blue" class="ui-button ui-widget ui-uom-default ui-corner-all ui-button-icon-only" role="button" aria-disabled="false" title="Close"><span class="ui-button-icon-primary ui-icon ../images/close_pop.png"></span><span class="ui-button-text">Close</span></button></a></div>
<?php }?>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Uom'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Uom<img src="images/sort.png" width="13" height="13" /></th>
			...
			<tr>
				...
				<td align="right">
				<a href="uom_ams.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
