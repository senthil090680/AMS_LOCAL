...
function colviewajax(page,params){   // For pagination and sorting of the Collection Deposited view page
	...
	$.ajax({
		url : "ajax_view_driver_allocate.php",
		type: "get",
		dataType: "text",
...
if(isset($_GET['delete_id']) && intval($_GET['delete_id'])) {
	...
	if(!mysql_query("delete FROM driver_allocate where id=$id")) {
		die('Error: ' . mysql_error());
	}
	$fgmembersite->RedirectToURL("view_driver_allocate.php?success=delete");
	// 
	//echo "hi";
	}
}
...
<div class="mcf"></div>
<div><h2 align="center">DRIVER ALLOCATION</h2></div> 

<div id="containerprforcd">

<span style="float:left;"><input type="button" name="kdproduct" value="Add Driver Allocation" class="buttonsbig1" onclick="window.location='driver_allocate.php'"></span><span style="float:right;"><input type="button" name="kdproduct" value="Close" class="buttons" onclick="window.location='ams_temp.php?id=3'"></span>
