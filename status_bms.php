...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `status_bms` where name like '%".$trimmed."%'";
}
else
{ 
	$qry="SELECT *  FROM `status_bms`"; 
}
...
$status=$_POST['status'];
$query = "SELECT * FROM status_bms where name = '$status'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("status_bms.php?success=exists");
		}
		else
		{
			...
			if ($status != "")
			{
			if(!mysql_query('INSERT INTO status_bms (name,created_by)VALUES ("'.$status.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("status_bms.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("status_bms.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($status != "")
			{
			if(!mysql_query('UPDATE status_bms SET name="'.$status.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("status_bms.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("status_bms.php?success=error");
...
{
$id=$_GET['id'];
$query = "SELECT * FROM status_bms where id=$id"; 
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Status</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100">Status*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='status' id='status'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='status' id='status'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Status<img src="images/sort.png" width="13" height="13" /></th>
			...
			<tr>
				...
				<td align="right">
				<a href="status_bms.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
