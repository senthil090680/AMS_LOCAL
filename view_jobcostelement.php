...
if($_REQUEST['cost_element']!='') {
	...
	$trimmed = trim($var);	
	$qry="SELECT jce.id AS JCEID,ce.name CENAME,j.job_desc AS JNAME FROM `jobcostelement` jce, costelement ce, jobs j WHERE jce.cost_elementid = ce.id AND jce.job_id = j.id AND ce.name LIKE '%".$trimmed."%'";
} else { 
	$qry="SELECT jce.id AS JCEID,ce.name CENAME,j.job_desc AS JNAME FROM `jobcostelement` jce, costelement ce, jobs j WHERE jce.cost_elementid = ce.id AND jce.job_id = j.id";
}
...
function delcall(id,name) {
	...
	if(confirmdata) {
		window.location = "view_jobcostelement.php?id="+id+"&del=del";
	}
}
function reqviewajax(page,params){   // For pagination and sorting of the Collection Deposited view page
	...
	$.ajax({
		url : "ajax_viewjobcostelement.php",
		type: "get",
		dataType: "text",
...
<div class="mcf"></div>
<div><h2 align="center">JOB-COST ELEMENTS</h2></div> 

<div id="containerprforcd">

<span style="float:left;"><input type="button" name="kdproduct" value="Add Job-Cost Elements" class="buttonsbigja" onclick="window.location='jobcostelement.php'"></span><span style="float:right;"><input type="button" name="kdproduct" value="Close" class="buttons" onclick="window.location='ams_temp.php?id=1'"></span>
...
<div class="clearfix"></div>
 ...
 <div class="clearfix"></div>
		...
		if($_GET['id']!='' && $_GET['del'] == 'del'){
			$query = "DELETE FROM `jobcostelement` WHERE id = $id";
			//Run the query
			$result = mysql_query($query) or die(mysql_error());
			header("location:view_jobcostelement.php?success=del");
		 }		
		?>
		<div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="reqviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Jobs<img src="images/sort.png" width="13" height="13" /></th>								
			...
			<tr>				
				...
				<td><?php echo $fgmembersite->upperstate($fetch['JNAME']); ?></td>
				<td nowrap="nowrap">
				<a href="edit_jobcostelement.php?id=<?php echo $fetch['JCEID'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="javascript:void(0);" onclick="delcall('<?php echo $fetch['JCEID']; ?>','<?php echo $fgmembersite->upperstate($fetch['CENAME']); ?>')" ><img src="images/trash.png" alt="" title="" width="11" height="11" /></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
