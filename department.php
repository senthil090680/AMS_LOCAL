...
$result = mysql_query($query);
		...
		{
			$fgmembersite->RedirectToURL("department.php?success=exists");
		}
		else
		{
			if ($department != "")
			{
			if(!mysql_query('INSERT INTO department (name,created_by)VALUES ("'.$department.'","'.$user_id.'")'))
			{
...
$result = mysql_query($query);
		...
		else
		{
			if ($department != "")
			{
			if(!mysql_query('UPDATE department SET name="'.$department.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
