...
function myFunction()
{
document.getElementById("costelement").value="";
document.getElementById("costelement").focus();
return false;
}
</script>
<script>
...
{
var costelementname=document.getElementById("costelement").value;
if(costelementname=="")
{
...
$costelement=$_POST['costelement'];
$query = "SELECT * FROM costelement where name = '$costelement'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("costelement.php?success=exists");
		}
		else
		{
			if ($costelement != "")
			{
			if(!mysql_query('INSERT INTO costelement (name,created_by)VALUES ("'.$costelement.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("costelement.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("costelement.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($costelement != "")
			{
			if(!mysql_query('UPDATE costelement SET name="'.$costelement.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("costelement.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("costelement.php?success=error");
...
{
$id=$_GET['id'];
$query = "SELECT * FROM costelement where id=$id"; 
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Cost Element</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100">Cost Element*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='costelement' id='costelement'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='costelement' id='costelement'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Cost Element'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<td align="right">
				<a href="costelement.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
