...
{
var costtypename=document.getElementById("costtype").value;
if(costtypename=="")
{
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `costtype` where name like '%".$trimmed."%'";
}
else
{ 
	$qry="SELECT *  FROM `costtype`"; 
}
...
$query = "SELECT * FROM costtype where name = '$costtype'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("costtype.php?success=exists");
		}
		else
		{
			if ($costtype != "")
			{
			if(!mysql_query('INSERT INTO costtype (name,created_by)VALUES ("'.$costtype.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("costtype.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("costtype.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($costtype != "")
			{
			if(!mysql_query('UPDATE costtype SET name="'.$costtype.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("costtype.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("costtype.php?success=error");
...
{
$id=$_GET['id'];
$query = "SELECT * FROM costtype where id=$id"; 
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Cost Type</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100">Cost Type*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='costtype' id='costtype'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='costtype' id='costtype'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Cost Type'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Cost Type<img src="images/sort.png" width="13" height="13" /></th>
			...
			<tr>
				...
				<td align="right">
				<a href="costtype.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
