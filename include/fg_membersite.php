...
class FGMembersite
{
    ...
    function RegisterUser()
    {
		...
		/* if(!$this->Sendmailtoadmin($formvars))
        {
            return false;
        }*/
...
*/
    ...
    function CheckLoginInDB($username,$password)
    {
		...
		$_SESSION['plainpassword']  = $row['plainpassword'];
    ...
    {
        $formvars['name'] = $this->Sanitize($_POST['firstname']);
        $formvars['email'] = $this->Sanitize($_POST['email']);
        $formvars['username'] = $this->Sanitize($_POST['username']);
        $formvars['password'] = $this->Sanitize($_POST['password']);
		$formvars['plainpassword'] = $this->Sanitize($_POST['password']);
		$formvars['usertype'] = $this->Sanitize($_POST['usertype']);
    }
 ...
 */
    ...
    function CreateTable()
    {
        $qry = "Create Table $this->tablename (".
				...
				"plainpassword VARCHAR( 32 ) NOT NULL ,".
    ...
    function InsertIntoDB(&$formvars)
    {
        ...
        $insert_query = 'insert into '.$this->tablename.'(
				...
				"' . $this->SanitizeForSQL($formvars['plainpassword']) . '",
				"' . $this->SanitizeForSQL($formvars['usertype']) . '",
                "' . $confirmcode . '"
                )';      
 ...
 /*
    ...
    */
	...
	function Sendmailtoadmin(&$formvars)
	{
...
$mail->From = $this->admin_email;
$mail->FromName = "Admin";
$mail->AddAddress($formvars['email']);
...
return true;
	...
	function getdbval($qryval,$resfieldname,$qrycolname,$tablename) { //PARAMETERS ARE TABLE QUERY COLUMN VALUE, TABLE RESULT COLUMN, TABLE QUERY COLUMN NAME, TABLE NAME
		$sel_qry				=	"SELECT $resfieldname FROM $tablename WHERE $qrycolname = '$qryval'";			
		$res_qry				=	mysql_query($sel_qry) or die(mysql_error());
		$row_qry				=	mysql_fetch_array($res_qry);
		return $qry				=	$row_qry[$resfieldname];
	}
