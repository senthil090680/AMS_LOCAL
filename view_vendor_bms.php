...
if(isset($_GET['delete_id']) && intval($_GET['delete_id'])) {
	...
	if(!mysql_query("delete FROM vendor_bms where id=$id")) {
		die('Error: ' . mysql_error());
	}
	$fgmembersite->RedirectToURL("view_vendor_bms.php?success=delete");
	// 
	//echo "hi";
	}
}
if($_REQUEST['searchname']!='') {
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM vendor_bms where name like '%".$trimmed."%'";
} else { 
	$qry="SELECT * FROM vendor_bms"; 
}
...
<div class="mcf"></div>
<div><h2 align="center">Vendor</h2></div> 

<div id="containerprforcd">

<span style="float:left;"><input type="button" name="kdproduct" value="Add vendor" class="buttonsbig1" onclick="window.location='vendor_bms.php'"></span><span style="float:right;"><input type="button" name="kdproduct" value="Close" class="buttons" onclick="window.location='ams_temp.php?id=2'"></span>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>				
				...
				<td >
				<a href="edit_vendor_bms.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
				<td>
				<a href="view_vendor_bms.php?delete_id=<?php echo $fetch['id'];?>&delete=1"><img src="images/trash.png" alt="" title="" width="11" height="11" onclick="return show_confirm('<?php echo $fetch['vendor_code']; ?>');"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
