...
$query_edit				=	"SELECT id,job_desc,job_code,job_type_id FROM jobs WHERE id = '$id'";			
...
if(isset($_POST['formsaveval']) && $_POST[formsaveval] == 800) {
	...
	if(!mysql_query('UPDATE jobs SET job_code="'.$job_code.'",job_type_id="'.$job_type_id.'",job_desc="'.$job_desc.'",updated_by="'.$user_id.'",updated_at=NOW() WHERE id = "'.$edit_id.'"')) {
			die('Error: ' . mysql_error());
		}
		$fgmembersite->RedirectToURL("view_jobs.php?success=update");
		echo "&nbsp;";
}
?>
...
<table width="50%" align="left">
 <tr>
  ...
  <tr height="30">
     ...
     <input type='text' name='job_code' id='job_code' tabindex="1" style="width:60px;" class="textbox" value="<?php echo $row_edit[job_code];?>" readonly="true" />
   </td>
	</tr>
    ...
    <tr height="30">
     ...
     <td><?php
		...
		while($row=mysql_fetch_array($result_state)) {
			if($row['id'] == $row_edit[job_type_id]){
 ...
 <tr>
  <td>
   ...
   <tr height="30">
	 ...
	 <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
     <td><input type='text' name='job_desc' id='job_desc' value="<?php echo $row_edit[job_desc]; ?>" size="52" tabindex="2" class="textbox"/>
     </td>
	</tr>
