...
{
var responsibilityname=document.getElementById("responsibility").value;
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `responsibility` where name like '%".$trimmed."%'";
}
else
{ 
	$qry="SELECT *  FROM `responsibility`"; 
}
...
$query = "SELECT * FROM responsibility where name = '$responsibility'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("responsibility.php?success=exists");
		}
		else
		{
			if ($responsibility != "")
			{
			if(!mysql_query('INSERT INTO responsibility (name,created_by)VALUES ("'.$responsibility.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("responsibility.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("responsibility.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($responsibility != "")
			{
			if(!mysql_query('UPDATE responsibility SET name="'.$responsibility.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("responsibility.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("responsibility.php?success=error");
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Responsibility Center</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100">Responsibility Center*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='responsibility' id='responsibility'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='responsibility' id='responsibility'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Responsibility Center'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Responsibility Center<img src="images/sort.png" width="13" height="13" /></th>
			...
			<tr>
				...
				<td align="right">
				<a href="responsibility.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
