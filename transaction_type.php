...
{
var transaction_typename=document.getElementById("transaction_type").value;
if(transaction_typename=="")
{
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `transaction_type` where name like '%".$trimmed."%'";
}
else
{ 
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Transaction Type</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100" nowrap="nowrap">&nbsp;Transaction Type*&nbsp;</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='transaction_type' id='transaction_type'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='transaction_type' id='transaction_type'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Transaction Type'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Transaction Type<img src="images/sort.png" width="13" height="13" /></th>
			...
			<tr>
				...
				<td align="right">
				<a href="transaction_type.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
