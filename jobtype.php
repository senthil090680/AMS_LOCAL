...
{
var jobtypename=document.getElementById("jobtype").value;
if(jobtypename=="")
{
...
if($_REQUEST['searchname']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `jobtype` where name like '%".$trimmed."%'";
}
else
{ 
...
$query = "SELECT * FROM jobtype where name = '$jobtype'"; 
$result = mysql_query($query);
		if (mysql_num_rows($result) > 0)
		{
			$fgmembersite->RedirectToURL("jobtype.php?success=exists");
		}
		else
		{
			if ($jobtype != "")
			{
			if(!mysql_query('INSERT INTO jobtype (name,created_by)VALUES ("'.$jobtype.'","'.$user_id.'")'))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("jobtype.php?success=create");
			}
			else
			{
			$fgmembersite->RedirectToURL("jobtype.php?success=error");
			}
		}
}
?>
...
$result = mysql_query($query);
		...
		else
		{
			if ($jobtype != "")
			{
			if(!mysql_query('UPDATE jobtype SET name="'.$jobtype.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
			{
			die('Error: ' . mysql_error());
			}
			$fgmembersite->RedirectToURL("jobtype.php?success=update");
			}
			else
			{
			$fgmembersite->RedirectToURL("jobtype.php?success=error");
...
{
$id=$_GET['id'];
$query = "SELECT * FROM jobtype where id=$id"; 
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headings">Job Type</div>
<div id="mytable" align="center">
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="validation"  onsubmit="return validateForm();">
<table>
  <tr height="60px">
     <td class="pclr" width="100">Job Type*</td>
     <td>
	 
	 <?php if(isset($_GET['id'])){ ?>
							<input type='text' name='jobtype' id='jobtype'  value="<?php echo $name; ?>" maxlength="50" autocomplete='off'/>
				
						<?php } else {?>
							<input type='text' name='jobtype' id='jobtype'  maxlength="50" autocomplete='off'/>
						<?php }?>
	 </td>
   </tr>
...
<div id="search">
        <input type="text" name="searchname" id="searchname" value="<?php echo $_REQUEST['searchname']; ?>" autocomplete='off' placeholder='Search By Jobtype'/>
...
<div class="mcf"></div>
       <div id="colviewajaxid">
			...
			<tr>
				...
				<th nowrap="nowrap" class="rounded" onClick="colviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');">Job Type<img src="images/sort.png" width="13" height="13" /></th>
			...
			<tr>
				...
				<td align="right">
				<a href="jobtype.php?id=<?php echo $fetch['id'];?>"><img src="images/user_edit.png" alt="" title="" width="11" height="11"/></a>
				</td>
			</tr>
			<?php $c++; $cc++; $slno++; }		 
